...
void mapper_function(int my_rank, KeyValue *kv , void *ms_ptr);
void reducer_function(char *key, int keybytes, char *multivalue, int nvalues, int *valuebytes, KeyValue *kv, void *ms_ptr);
...
int main(int narg, char *args[])
{
  ...
  float **stochastic = (float **) malloc(num_page*sizeof(float *));
  int total_connections = to.size();
	vector<double> I_prev;
	vector<double> I;
  for (int i = 0; i < num_page; i++)
  {
    stochastic[i] = (float *) malloc(num_page*sizeof(float));
		I.push_back(0.0);
  ...
  ms->stochastic_matrix = stochastic;
  // for (int i = 0; i < num_page; i++)
  // {
...
void mapper_function(int my_rank, KeyValue *kv , void *ms_ptr){
  ...
  for (int i = endpoint(my_rank, ms->nprocs, ms->num_page); i < endpoint(my_rank+1, ms->nprocs, ms->num_page); i++){
    ...
    for (int j = 0; j < ms->num_page; j++){
      if (product.size() <= ms->num_page){
        product.push_back(0);
      }
      product[j] += ::beta/ms->num_page * ms->I_prev[i] + (1-::beta)*ms->stochastic_matrix[i][j]*ms->I_prev[i];
      if (i==endpoint(my_rank+1, ms->nprocs, ms->num_page) - 1){
        kv->add((char *) &j, sizeof(int), (char *) &product[j], sizeof(double));
      }
    }
  }
}
...
void reducer_function(char *key, int keybytes, char *multivalue, int nvalues, int *valuebytes, KeyValue *kv, void *ms_ptr){
  mr_struct *ms = (mr_struct*) ms_ptr;
  double I_new_for_key = 0;
  double *combined_vals = (double*) multivalue;
  int key_int = *((int *) key);

  if (key_int > ms->assigned_end_r)
		ms->assigned_end_r = key_int;
	if (key_int < ms->assigned_start_r)
		ms->assigned_start_r = key_int;

  for (int i = 0; i < nvalues; i++)  {
    double val = *(combined_vals + i);
    I_new_for_key += val;
  }
  ms->I[key_int] = I_new_for_key;
}
